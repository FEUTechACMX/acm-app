...
const Carousel: React.FC<CarouselProps> = ({
	...
	useEffect(() => {
		...
		glide.on(["mount.after", "run"], () => {
			...
			const slideElements = document.querySelectorAll(`#slide-image-${id}`);
						...
						<div className="col-span-1 flex flex-col md:ms-4">
							<div className="flex justify-left">
								<h1 className="hall-of-fame-header font-bold md:text-xl flex items-center">
									EXPO&nbsp;
									<span className="text-[var(--coral-pink)]">2023</span>
									&nbsp;
									<span className="font-bold text-xs md:text-sm">2.0</span>
								</h1>
							</div>
								...
								{slides.map((slide) => (
									<li className="glide__slide flex flex-col items-center justify-center">
										...
										<div className="mt-4 p-4 sm:p-0">
											<div className="font-bold text-lg">{slide.ranking}</div>
											<div className="font-bold text-3xl">{slide.group}</div>
											<p>{slide.thesis}</p>
